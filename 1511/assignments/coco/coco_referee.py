...
import argparse, atexit, codecs, collections, copy, getpass, glob, io, os, random, re, shutil, signal, subprocess, sys, time, tempfile, zipfile
...
def args_parser():
    ...
    parser.add_argument("--colorize", action="store_true", default=sys.stdout.isatty(), help="show test input")
    ...
    parser.add_argument("-d", "--debug", action="count", default=0,  help="show debug input")
...
def print_play(round, player, card, illegal_explanation):
    ...
    if illegal_explanation:
        print("{illegal_explanation}: {ILLEGAL_PLAY_PENALTY} penalty points - referee chooses {card} ", end='')
    else:
...
def run_binary_for_play(player, cards_played_current_round, cards_played_previous_rounds):
    ...
    stdout = run_binary(player, input=input)
    if stdout and not re.match(r'^\s*\d\d\s*$', stdout) and args.show_player_stdout:
    ...
    if numbers:
        ...
        card = ''
        explanation = 'program output not an integer'
    ...
    if stderr and exit_status != 0:
        ...
        if args.stop_on_error:
            ...
            if binary.endswith('valgrind'):
                print(colored(f"dcc --valgrind {player['source']} -o coco", 'red'))
            else:
                print(colored('dcc coco.c -o coco', 'red'))
            print(colored('echo -e "{}"|./coco'.format(input.rstrip().replace('\n', '\\n')), 'red'))
            print()
