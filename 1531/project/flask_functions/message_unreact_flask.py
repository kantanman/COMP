...
def unreact_message(token, message_id, react_id):
    ...
    # var1 checks if message_id belongs to a valid message
    var1 = False
    # var2 checks if messages is already unreacted to
    var2 = False
    ...
    for channel in lsdata.users[u_id].channels_in:
        for message in channel.messages:
            if message_id == message[message_id]:
                var1 = True
                if  message[message_id].reaction[u_id] is react_id:
                    message[message_id].reaction.remove(u_id)
                    var2 = True
                    break
