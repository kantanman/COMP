...
static void execute (char **args, char **path, char **envp)
{
    ...
    // check first argument and see if it is executable
    if ((arg[0] == '.') || (arg[0] == '/')) {
        if (isExecutable(arg))
            file = args[0];
    } else { // look for an executable
        for (int i = 0; path[i] != NULL; i++ ) {
            char *PATH = malloc(strlen(args[0]) + strlen(path[i]) + 1);
	    strcat(PATH, path[i]);
            strcat(PATH, "/");
            strcat(PATH, args[0]);
    ...
    if (file == NULL) {
        ...
        printf("%s\n", file);
        int check = execve(file, args, envp);
        if (check == -1)
