...
isIdent_row_cond:
	bge	$s2, $s1 ret_1	# when row > n, it is an identiy matrix
        j isIdent_column_cond


isIdent_column_cond:
        bge	$s3, $s1 col_restart	#row>n?
        beq     $s3,$s2 cond_1  #col == row, go to cond_1
        bne     $s3,$s2 cond_2  #col != row, go to cond_2
...
col_restart:        #restart the column counter
        li      $s3, 0
        j       isIdent_row_step
...
cond_2:    #if row and column are not equal, execute
        mul     $t0, $s2, $s1   #value of the index
        add     $t0, $t0, $s3   #+ col
        li      $t1, 4    
        mul     $t0, $t0, $t1   #*sizeof(word)    
