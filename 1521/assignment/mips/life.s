...
cond_1:
        li    $t0, 1
        beq   $s0, $t0 nn_cond     # check if old is equal to 1
        nop
        j cond_2

cond_2:
        li    $t0, 3
        beq   $s1, $t0 return_1    # check if nn is equal to 3
        nop
        j return_0
...
nn_cond:
        li    $t0, 2
        li    $t1, 3
        blt   $s1, $t0 return_0     # if nn < 2, return 0
        nop
        beq   $s1, $t0 return_1     # if nn = 2, return 1
        nop
        beq   $s1, $t1 return_1     # if nn = 3, return 1
        nop
        j return_0
...
execute:
        mul   $t0, $s0, $s2           # calculate the index
        add   $t0, $t0, $s1
        lb    $t1, newBoard($t0)      # load state of index from newboard into $t1
        sb    $t1, board($t0)         # store index from $t1 into board
